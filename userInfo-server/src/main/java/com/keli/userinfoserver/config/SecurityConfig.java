...
public class SecurityConfig {
    ...
    @Bean
    @Order(2)
    public SecurityFilterChain filterChain(HttpSecurity http, JwtDecoder jwtDecoder,RoleHierarchy roleHierarchy) throws Exception {
        DefaultWebSecurityExpressionHandler handler = new DefaultWebSecurityExpressionHandler();
        handler.setRoleHierarchy(roleHierarchy);
        http.setSharedObject(SecurityExpressionHandler.class,handler);

        http.securityMatcher("/api/userinfo/**")
                .authorizeHttpRequests(auth -> auth
                        .anyRequest().hasAuthority("SCOPE_userinfo_server")
                )
